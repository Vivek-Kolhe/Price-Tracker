...
urls = ["https://www.amazon.in/Test-Exclusive-547/dp/B078BNQ318/ref=pd_lpo_107_t_1/259-7042788-8081138?_encoding=UTF8&pd_rd_i=B078BNQ318&pd_rd_r=18883baa-d31a-48c1-afb9-efaae994b163&pd_rd_w=sC6sf&pd_rd_wg=GCLUL&pf_rd_p=5a903e39-3cff-40f0-9a69-33552e242181&pf_rd_r=C6S449FK1YR9WYB4ZGRE&psc=1&refRID=C6S449FK1YR9WYB4ZGRE",
		...
		"https://www.amazon.in/Apple-iPhone-Xs-256GB-Silver/dp/B07J3F1M5F/ref=nav_ya_signin?dchild=1&keywords=iphone+11&qid=1595926838&sr=8-2-spons&psc=1&spLa=ZW5jcnlwdGVkUXVhbGlmaWVyPUFPMkFBQkJGQk1WTzImZW5jcnlwdGVkSWQ9QTAyMjUwMzkxU1JJU1FXMDIwTjBMJmVuY3J5cHRlZEFkSWQ9QTEwMDA3OTMxQkU1QkY1R0pHODNGJndpZGdldE5hbWU9c3BfYXRmJmFjdGlvbj1jbGlja1JlZGlyZWN0JmRvTm90TG9nQ2xpY2s9dHJ1ZQ==&",
...
exp_prices = [] # Will notify if the item gets below these prices, must enter corresponding to urls
items = []
...
while True:
	...
	for i in range(len(items)):
		if items[i]["Price"] < exp_prices[i]:
			msg = f"Price has lowered for {items[i]['Title']} to Rs {items[i]['Price']}.\nGo check it out at {items[i]['Url']}"
			send_mail(msg)
			# print("done")
	time.sleep(6*3600)
	items.clear()
